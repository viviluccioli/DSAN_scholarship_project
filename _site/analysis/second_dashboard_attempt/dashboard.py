...
def create_quarto_compatible_dashboard(df):
    # Get all possible unknown count values
    unknown_counts = sorted(df['unknown_count'].unique())
    
    # Create the HTML structure - WITHOUT html, head, body tags
    html = """
    ...
    # Return a dictionary with both the HTML content and JS data
    return {
        "html_content": html,
        "dashboard_data": json.dumps(data_json)
    }

def create_quarto_dashboard_files(df, output_html_path, output_css_path, output_js_path):
    # First create compatible dashboard
    dashboard_data = create_quarto_compatible_dashboard(df)
    
    # Create HTML file (without headers, just content)
    with open(output_html_path, 'w') as f:
        f.write("---\ntitle: \"Interactive Dashboard\"\nformat: html\n---\n\n")
        f.write("<link rel=\"stylesheet\" href=\"dashboard_styles.css\">\n")
        ...
        f.write("<script src=\"dashboard.js\"></script>\n\n")
        f.write(dashboard_data["html_content"])
        f.write("\n<script>const dashboardData = " + dashboard_data["dashboard_data"] + ";</script>\n")
...
if __name__ == "__main__":
    ...
    create_quarto_dashboard_files(
        df, 
        output_html_path='analysis/interactive_dashboard.qmd',  # Note: Use .qmd extension
        output_css_path='analysis/dashboard_styles.css',
        output_js_path='analysis/dashboard.js'
    )